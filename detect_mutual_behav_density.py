...
def _plot_mutual_face_fixation_density(eye_mvm_behav_df, sparse_nan_removed_sync_gaze_df, params):
    ...
    - sparse_nan_removed_sync_gaze_df: DataFrame containing synchronized gaze data with neural timeline.
    ...
    for session_name, session_df in tqdm(session_groups, desc="Processing sessions"):
        ...
        for ax, run_number in zip(axes, runs):
            ...
            m1_fix_intervals = np.array(m1_data['fixation_start_stop'].iloc[0])
            m1_fix_locations = m1_data['fixation_location'].iloc[0]
            m2_fix_intervals = np.array(m2_data['fixation_start_stop'].iloc[0])
            ...
            window_size = max(5 * avg_fix_duration, 100)  # Ensure window size is at least 100
